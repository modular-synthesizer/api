module Modusynth
  module Services
    class Synthesizers < Modusynth::Services::Base
      ...
      def build account:, name: nil, voices: 1, sample_rate: 44_100, **_
        instance = model.new(name:, voices:, sample_rate:)
      ...
      def update synthesizer, **payload
        ...
        if synthesizer.creator.account.id == session.account.id
          synthesizer.update(**payload.slice(:name, :racks, :slots, :voices, :sample_rate))
          return synthesizer unless synthesizer.valid?
        end
        membership = synthesizer.memberships.where(account: session.account).first
        membership.update!(**payload.slice(:x, :y, :scale)) unless membership.nil?
        synthesizer
      end
