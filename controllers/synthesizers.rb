...
module Modusynth
  module Controllers
    class Synthesizers < Modusynth::Controllers::Base
      endpoint 'post', '/:uuid/synthesizers', right: ::Rights::SYNTHESIZERS_WRITE do
        synthesizer = service.create(account:, **symbolized_params)
        membership = synthesizer.guest(account:)
        render_json 'synthesizers/_synthesizer.json', status: 201, membership:
      end

      endpoint 'delete', '/:uuid/synthesizers/:id', right: ::Rights::SYNTHESIZERS_WRITE do
        ...
        service.remove(account:, **symbolized_params)
        halt 204
      end

      endpoint 'get', '/:uuid/synthesizers', right: ::Rights::SYNTHESIZERS_READ do
        memberships = service.list(account, **symbolized_params)
        render_json 'synthesizers/list.json', memberships:
      end

      endpoint 'get', '/:id', right: ::Rights::SYNTHESIZERS_READ do
      ...
      endpoint 'put', '/:id', right: ::Rights::SYNTHESIZERS_WRITE do
