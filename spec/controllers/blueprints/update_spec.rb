RSpec.describe 'PUT /:id' do
  ...
  describe 'Nominal case' do
    describe 'When nothing is updated' do
      before do
        put "/#{blueprint.id.to_s}", {auth_token: session.token}
      end
  ...
  describe 'Alternative cases' do
    describe 'Update the name' do
      before do
        put "/#{blueprint.id.to_s}", {auth_token: session.token, name: 'OtherName'}
      end
      it 'Returns a 200 (OK) status code' do
        expect(last_response.status).to be 200
      end
    ...
    describe 'Update the slots' do
      before do
        put "/#{blueprint.id.to_s}", {auth_token: session.token, slots: 42}
      end
      it 'Returns a 200 (OK) status code' do
        expect(last_response.status).to be 200
      end
    ...
    describe 'Update the category' do
      ...
      describe 'The update category' do
        before { blueprint.reload }

        it 'Has the correct category' do
          expect(blueprint.category.id).to eq category.id
        end
      end
    end
  end
  ...
  describe 'Error cases' do
    describe 'Blueprint not found' do
      before do
