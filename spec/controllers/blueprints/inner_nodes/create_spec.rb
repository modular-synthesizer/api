RSpec.describe 'POST /blueprints/nodes' do
  def app
    Modusynth::Controllers::Blueprints::InnerNodes
  end
  ...
  describe 'Nominal case' do
    ...
    describe 'The created inner node' do
      let!(:node) { blueprint.reload; blueprint.inner_nodes.last }
  ...
  describe 'Alternative cases' do
    describe 'When the X coordinate is not given' do
      ...
      it 'Has filled the remaining coordinate with the correct default value' do
        blueprint.reload
        expect(blueprint.inner_nodes.last.x).to be 0
      end
    end
    describe 'When the Y coordinate is not given' do
      ...
      it 'Has filled the remaining coordinate with the correct default value' do
        blueprint.reload
        expect(blueprint.inner_nodes.last.y).to be 0
      end
    end
  end
  describe 'Error cases' do
    describe 'When the name is not given' do
      ...
      it 'Has created no new inner node' do
        blueprint.reload
        expect(blueprint.inner_nodes.where(generator: 'NodeGenerator').count).to be 0
      end
    end
    ...
    describe 'When the blueprint UUID is not given' do
      before do
        post '/', {
          auth_token: session.token,
          name: 'inner-node',
