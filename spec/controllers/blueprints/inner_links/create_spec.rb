RSpec.describe 'POST /blueprints/links' do
  ...
  describe 'Nominal case' do
    ...
    describe 'The created link' do
      let!(:link) { blueprint.reload; blueprint.inner_links.last }
  ...
  describe 'Alternative case' do
    describe 'The origin index is not given' do
      ...
      it 'Has filled the index with the default value' do
        blueprint.reload
        expect(blueprint.inner_links.last.from.index).to be 0
      end
    end
    describe 'The destination index is not given' do
      ...
      it 'Has filled the index with the default value' do
        blueprint.reload
        expect(blueprint.inner_links.last.to.index).to be 0
      end
    end
  end
  describe 'Error cases' do
    describe 'The origin payload is not given' do
      ...
      it 'Has created no new link' do
        expect(blueprint.inner_links.count).to be 0
      end
    end
    ...
    describe 'The blueprint UUID is not given' do
      before do
        post '/', {
          auth_token: session.token,
          from: { node: 'from-node', index: 1 },
    ...
    describe 'The blueprint is not found' do
      before do
        post '/', {
          auth_token: session.token,
